...
class ApiController
{
    ...
    private static function sitemapItems(): array
    {
        ...
        foreach (config('locales') as $loc) {
            ...
            foreach (Repo::projects(null, true, 1, 100)['items'] as $p) $items[] = ['url' => $base . '/' . $loc . '/projects/' . $p['id'], 'lastModified' => now(), 'changeFrequency' => 'monthly', 'priority' => 0.5];
            foreach (Repo::products(null, null, true, null, 1, 100)['items'] as $p) $items[] = ['url' => $base . '/' . $loc . '/products/' . $p['id'], 'lastModified' => now(), 'changeFrequency' => 'monthly', 'priority' => 0.5];
            foreach (Repo::blogPosts(true, null, 1, 100)['items'] as $b) $items[] = ['url' => $base . '/' . $loc . '/blog/' . $b['slug'], 'lastModified' => now(), 'changeFrequency' => 'monthly', 'priority' => 0.5];
        }
        return $items;
    }
