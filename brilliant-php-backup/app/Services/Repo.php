...
class Repo
{
    ...
    {
        foreach (['id','categoryId','typeId','year','brandId','order'] as $f) {
    ...
    public static function services(?int $categoryId = null, ?bool $onlyActive = null, ?string $search = null, int $pageIndex = 1, int $pageSize = 10): array
    {
        ...
        $data = 'SELECT s.*, sc.name AS category_name FROM services s LEFT JOIN service_categories sc ON sc.id = s.category_id' . $where . ' ORDER BY s.sort_order ASC, s.id ASC';
    ...
    {
        $row = Database::fetchOne('SELECT s.*, sc.name AS category_name FROM services s LEFT JOIN service_categories sc ON sc.id = s.category_id WHERE s.id = ?', [$id]);
    ...
    public static function products(?int $brandId = null, ?int $categoryId = null, ?bool $onlyActive = null, ?string $search = null, int $pageIndex = 1, int $pageSize = 10): array
    {
        ...
        $from = ' FROM products p LEFT JOIN product_categories pc ON pc.id = p.category_id LEFT JOIN product_brands pb ON pb.id = pc.brand_id';
        $count = 'SELECT COUNT(*) ' . $from . $where;
        $data = 'SELECT p.*, pc.name AS category_name, pc.brand_id, pb.name AS brand_name ' . $from . $where . ' ORDER BY p.sort_order ASC, p.id ASC';
        $result = self::paged($count, $data, $params, $pageIndex, $pageSize);
        $result['items'] = array_map(fn($r) => self::intify(self::camelRow($r)), $result['items']);
        return $result;
    }
    ...
    {
        $row = Database::fetchOne('SELECT p.*, pc.name AS category_name, pc.brand_id, pb.name AS brand_name FROM products p LEFT JOIN product_categories pc ON pc.id = p.category_id LEFT JOIN product_brands pb ON pb.id = pc.brand_id WHERE p.id = ?', [$id]);
        return $row === null ? null : self::intify(self::camelRow($row));
    }
    ...
    // ---- blog ----
    public static function blogPosts(?bool $publishedOnly = null, ?int $tagId = null, int $pageIndex = 1, int $pageSize = 10): array
    {
        $where = ' WHERE 1=1';
        $params = [];
