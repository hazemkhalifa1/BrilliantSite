...
if (!in_array($entity, ['team', 'clients'], true)) $skip[] = 'order';
...
<div class="mx-auto max-w-3xl space-y-6">
  ...
  <form method="post" action="<?= e($action) ?>" class="card-brilliant space-y-5 bg-white p-6" data-admin-form>
    ...
    <?php foreach ($fieldDefs as $f): $name = $f['name']; $type = $f['type']; if (in_array($name, $skip, true)) continue; $label = t($f['label'] ?? $name); $req = !empty($f['required']); $val = $f['value'] ?? ''; $ph = $placeholderFor($f); ?>
