      ...
      <thead>
        <tr class="border-b border-line bg-neutral-light">
          ...
          switch ($entity) {
            ...
            case 'testimonials': $headerKeys = ['client', 'role', 'order', 'status']; break;
                ...
                <td class="px-4 py-3 align-middle">
                  <div class="flex items-center gap-3">
                    <?php if (!empty($item['imagePath'])): ?>
                      <img src="<?= e(\App\Services\Repo::getImagePath($item, 'imagePath')) ?>" alt="<?= e(lpair($item['name'], $item['nameAr'] ?? null)) ?>" class="h-10 w-10 shrink-0 rounded-full border border-line object-cover">
                    <?php else: ?>
                      <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-line bg-tertiary font-headline text-xs font-bold text-white"><?= e(strtoupper(mb_substr(trim((string)($item['name'] ?? '')), 0, 1))) ?></span>
                    <?php endif; ?>
                    <div>
                      <a href="<?= e(admin_url('/testimonials/'.$item['id'])) ?>" class="font-semibold text-neutral transition-colors hover:text-secondary"><?= e(lpair($item['name'], $item['nameAr'] ?? null)) ?></a>
                      <p class="mt-0.5 line-clamp-1 max-w-sm text-xs text-neutral/50"><?= e($truncate(lpair($item['quote'], $item['quoteAr'] ?? null))) ?></p>
                    </div>
                  </div>
                </td>
                <td class="px-4 py-3 align-middle text-neutral/80"><?= e(lpair($item['role'], $item['roleAr'] ?? null) ?: '—') ?></td>
                <td class="px-4 py-3 align-middle text-neutral/80"><?= e($item['order'] ?? '—') ?></td>
                <td class="px-4 py-3 align-middle"><?= $badge((bool)$item['isActive']) ?></td>
              <?php else: ?>
