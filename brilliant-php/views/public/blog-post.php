...
// Legacy auto-suffixed slugs ("…-2") now 301 to the clean canonical form, so
// old indexed links never 404 and search engines converge on one URL.
$canonSlug = Repo::canonicalSlug($post['slug']);
if ($canonSlug !== $slug) {
    redirect('/' . $locale . '/blog/' . $canonSlug, 301);
}
$post['slug'] = $canonSlug;
...
// og:image = the post cover when set, otherwise the site logo (never a broken
// placeholder). The layout keeps the logo as its default fallback.
$hasCover = trim((string)($post['coverImagePath'] ?? '')) !== '';
if ($hasCover) {
...
$related = [];
if ($firstTagId) {
  $rel = Repo::blogPosts(true, $firstTagId, 1, 3, (int)$post['id'])['items'];
  foreach ($rel as $item) {
...
if (count($related) < 3) {
  $latest = Repo::blogPosts(true, null, 1, 3, (int)$post['id'])['items'];
...
<article class="card-brilliant group flex flex-col overflow-hidden transition-colors hover:border-tertiary">
  ...
  <div class="flex flex-1 flex-col p-5">
    ...
    <a href="<?= e(lnk('/blog/' . Repo::canonicalSlug($post['slug']))) ?>" class="mt-4 inline-flex items-center gap-1.5 font-headline text-xs font-semibold uppercase tracking-wide text-secondary transition-colors hover:text-tertiary"><?= e(t('common.readMore')) ?><?= $arrowRightSvg ?></a>
  </div>
</article>
<?php
};
...
<?= json_encode([
  ...
  'image' => $hasCover ? Repo::getImagePath($post, 'coverImagePath') : $siteUrl . '/logo.png',
...
<header class="relative flex min-h-[64vh] w-full items-center overflow-hidden bg-neutral md:min-h-[72vh]">
  ...
  <div class="container-brilliant relative z-10 w-full py-32 md:py-40">
    <div class="mx-auto max-w-3xl text-center">
      ...
      <div class="mt-8 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm text-white/80 reveal-on-scroll" style="transition-delay: 80ms">
      ...
      <?php if (!empty($post['tags']) && count(array_filter($post['tags'])) > 0): ?>
        <div class="mt-8 flex flex-wrap items-center justify-center gap-2 reveal-on-scroll" style="transition-delay: 200ms">
  ...
  <div class="container-brilliant">
    ...
    <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <?php foreach ($related as $i => $item): ?>
        <div class="pop-card h-full reveal-on-scroll" style="transition-delay: <?= $i * 90 + 100 ?>ms" data-from="<?= $i % 2 === 0 ? 'left' : 'right' ?>">
          <?php $renderBlogCard($item, $locale); ?>
        </div>
      <?php endforeach; ?>
    </div>
  </div>
