...
$icons = [
    ...
    'target'     => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-8 w-8 transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>',
    'gem'        => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-8 w-8 transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110"><path d="M6 3h12l4 6-10 13L2 9Z"/><path d="M11 3 8 9l4 13 4-13-3-6"/><path d="M2 9h20"/></svg>',
    'handshake'  => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-8 w-8 transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110"><path d="m11 17 2 2a1 1 0 1 0 3-3"/><path d="m14 14 2.5 2.5a1 1 0 1 0 3-3l-3.88-3.88a3 3 0 0 0-4.24 0l-.88.88a1 1 0 1 1-3-3l2.81-2.81a5.79 5.79 0 0 1 7.06-.87l.47.28a2 2 0 0 0 1.42.25L21 4"/><path d="m21 3 1 11h-2"/><path d="M3 3 2 14l6.5 6.5a1 1 0 1 0 3-3"/><path d="M3 4h8"/></svg>',
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <?php foreach ($INDUSTRIES as $index => $industry): ?>
          <div class="reveal-on-scroll" style="transition-delay: <?= $index * 80 ?>ms">
            <div class="group flex h-full flex-col border-2 border-[#111C2D] bg-white p-8 transition-transform duration-300 hover:-translate-y-1.5 hover:-rotate-1">
              <div class="mb-6 flex h-14 w-14 items-center justify-center bg-[#0059BB] text-white transition-colors group-hover:bg-[#BA1A1A]"><div class="transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110"><?= $industry['icon'] ?></div></div>
    ...
    <div class="container-brilliant relative z-10">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <?php foreach ($PILLARS as $index => $pillar): ?>
          <div class="reveal-on-scroll" style="transition-delay: <?= $index * 100 ?>ms" data-from="<?= $index % 2 === 0 ? 'left' : 'right' ?>">
            <div class="h-full transition-transform duration-300 hover:-translate-y-1.5">
              <div class="pop-card group flex h-full flex-col border-2 border-white/20 bg-white/5 p-10 transition-colors duration-300 hover:border-[#BA1A1A]" style="transition-delay: <?= $index * 100 + 120 ?>ms">
    ...
    <div class="container-brilliant">
      <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <?php foreach ($STATS as $index => $stat): ?>
          <div class="reveal-on-scroll" style="transition-delay: <?= $index * 80 ?>ms">
            <div class="border-2 border-[#111C2D] bg-white p-8 text-center shadow-[4px_4px_0px_0px_#111C2D] transition-transform duration-300 hover:-translate-y-1.5 hover:-rotate-1">
              <p class="font-headline text-4xl font-bold md:text-5xl <?= $index === count($STATS) - 1 ? 'text-[#BA1A1A]' : 'text-[#0059BB]' ?>"><span class="count-up-value" data-countup="<?= e($stat['value']) ?>" dir="ltr"><?= e($stat['value']) ?></span></p>
    ...
    <div class="container-brilliant">
      <div class="grid items-center gap-12 lg:grid-cols-2">
        <div class="reveal-on-scroll">
          ...
          <div class="group relative overflow-hidden border-2 border-[#111C2D] bg-[#111C2D] text-white shadow-[6px_6px_0px_0px_#0059BB]">
            <div class="absolute right-4 top-4 font-headline text-8xl italic leading-none text-white opacity-10 transition-transform duration-500 group-hover:-rotate-6 group-hover:scale-110"><?= e($founder ? $initials(localized($locale, $founder['name'], $founder['nameAr'] ?? null)) : 'AG') ?></div>
