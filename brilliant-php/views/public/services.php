...
<div>
  <section class="relative overflow-hidden bg-[#111C2D] pb-24 pt-32 md:pb-32 md:pt-48">
    ...
    <div class="relative z-20 mx-auto w-full max-w-4xl px-4 text-center sm:px-6 lg:px-8">
      ...
      <a href="#core" class="group inline-flex items-center gap-2 border-2 border-[#111C2D] bg-white px-10 py-4 font-headline text-sm font-bold uppercase tracking-widest text-[#111C2D] shadow-[6px_6px_0px_0px_#0059BB] transition-transform hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_#0059BB]">
        <?= e($s['exploreCapabilities']) ?> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5 transition-transform duration-300 group-hover:translate-y-1"><path d="M12 5v14"/><path d="m19 12-7 7-7-7"/></svg>
      </a>
    </div>
  </section>
  ...
  <section id="core" class="bg-white py-24 md:py-32">
    <div class="container-brilliant">
      ...
      <div class="space-y-24 md:space-y-32 loadmore-container">
        <?php foreach ($serviceItems as $index => $service): ?>
          <div class="loadmore-item flex flex-col items-center gap-10 md:flex-row md:gap-16<?= $index % 2 === 1 ? ' md:flex-row-reverse' : '' ?><?= $index >= 3 ? ' hidden' : '' ?>">
            <div class="reveal-on-scroll w-full md:w-1/2">
              <div class="relative overflow-hidden border-2 border-[#111C2D] bg-[#111C2D] shadow-[6px_6px_0px_0px_#BA1A1A] group">
                <?php if (!empty($service['iconPath'])): ?>
                  <div class="h-72 w-full md:h-96">
                    <img src="<?= e(Repo::getImagePath($service, 'iconPath')) ?>" alt="<?= e(localized($locale, $service['title'], $service['titleAr'] ?? null)) ?>" loading="eager" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105">
                  </div>
                <?php else: ?>
                  <div class="h-72 w-full bg-cover bg-center opacity-40 mix-blend-luminosity transition-transform duration-700 group-hover:scale-105 md:h-96" style="background-image: url('<?= e($FALLBACK_IMAGE) ?>')"></div>
                <?php endif; ?>
            ...
            <div class="reveal-on-scroll w-full md:w-1/2" style="transition-delay: 100ms">
              ...
              <?php if (!empty($service['relatedBlogPostId']) && !empty($service['relatedBlogSlug'])): ?>
                <a href="<?= e(lnk('/blog/' . Repo::canonicalSlug($service['relatedBlogSlug']))) ?>" class="mt-6 inline-block border-s-4 border-[#0059BB] bg-white px-5 py-3 text-sm font-semibold text-[#111C2D] shadow-[3px_3px_0px_0px_#0059BB] transition-transform hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[5px_5px_0px_0px_#BA1A1A]">
                  <?= e($sb['relatedGuide']) ?> <span class="font-bold text-[#BA1A1A]"><?= e(localized($locale, $service['relatedBlogTitle'] ?? '', $service['relatedBlogTitleAr'] ?? null)) ?></span>
                </a>
              <?php endif; ?>
            </div>
          </div>
      ...
      <div class="mt-20 text-center">
        <button type="button" data-loadmore class="loadmore-btn group inline-flex items-center gap-2 border-2 border-[#111C2D] bg-white px-10 py-4 font-headline text-sm font-bold uppercase tracking-widest text-[#111C2D] shadow-[4px_4px_0px_0px_#0059BB] transition-transform hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_#0059BB]">
          <?= e($sb['showMore']) ?> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5 transition-transform duration-300 group-hover:translate-y-1"><path d="m6 9 6 6 6-6"/></svg>
        </button>
      </div>
      <?php endif; ?>
    </div>
  ...
  <section class="bg-[#F9F9FF] py-24 md:py-32" style="<?= e($surfaceGridStyle) ?>">
    <div class="mx-auto w-full max-w-3xl px-4 sm:px-6 lg:px-8">
      <div class="reveal-on-scroll">
        <div class="pop-card group border-2 border-[#111C2D] bg-white p-10 text-center shadow-[6px_6px_0px_0px_#111C2D] md:p-14">
          ...
          <a href="<?= e(lnk('/contact')) ?>" class="mt-10 inline-flex items-center gap-3 bg-[#BA1A1A] px-10 py-4 font-headline text-base font-bold uppercase tracking-widest text-white shadow-[4px_4px_0px_0px_#111C2D] transition-transform hover:-translate-x-1 hover:-translate-y-1">
            <?= e($s['initiateProtocol']) ?> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5 transition-transform duration-300 group-hover:translate-x-1.5"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
          </a>
        </div>
      </div>
    </div>
