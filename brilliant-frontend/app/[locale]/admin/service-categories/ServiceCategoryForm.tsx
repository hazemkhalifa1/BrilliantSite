...
export function ServiceCategoryForm({ initial }: ServiceCategoryFormProps) {
  const router = useRouter();
  const t = useTranslations("admin.form");
  const section = useTranslations("admin.sections.serviceCategories");
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (initial) {
        await apiFetch(`/service-categories/${initial.id}`, {
          method: "PUT",
          body: JSON.stringify({ id: initial.id, ...body }),
        });
      } else {
        await apiFetch("/service-categories", { method: "POST", body: JSON.stringify(body) });
      }
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="card-brilliant space-y-5 bg-white p-6">
        ...
        <Input
          ...
          placeholder={t("serviceCategoryNamePlaceholder")}
        />
        ...
        <TextArea
          ...
          placeholder={t("serviceCategoryDescriptionPlaceholder")}
        />
