...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "admin.sections.services" });
...
export default async function ServicesAdminPage({ searchParams }: ServicesAdminPageProps) {
  const t = await getTranslations("admin.sections.services");
  ...
  const [data, categories] = await Promise.all([
    adminFetchList<Service>(`/services?${query.toString()}`).catch(() => ({ items: [], totalCount: 0 })),
    adminFetchList<ServiceCategory>("/service-categories?pageSize=100").catch(() => ({
      items: [],
      totalCount: 0,
    })),
  ]);
  ...
  const columns: Column<Service>[] = [
    {
      ...
      cell: (service) => (
        <div>
          ...
          <p className="mt-0.5 text-xs text-neutral/50">{truncate(service.description, 70)}</p>
        </div>
      ),
    },
    {
    ...
    { key: "order", header: t("headers.order"), cell: (service) => service.order ?? "—" },
    {
      key: "status",
      header: t("headers.status"),
      cell: (service) => (
        <Badge variant={service.isActive ? "success" : "neutral"}>
          {service.isActive ? common("active") : common("inactive")}
        </Badge>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        action={
          <Link href="/admin/services/new" className="btn-primary !px-4 !py-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <Link
            href={search ? `/admin/services?search=${encodeURIComponent(search)}` : "/admin/services"}
        ...
        <form
          method="get"
          action={`/admin/services${categoryId ? `?categoryId=${categoryId}` : ""}`}
        ...
        >
          ...
          {search && (
            <Link
              href={categoryId ? `/admin/services?categoryId=${categoryId}` : "/admin/services"}
        ...
        actions={(service) => (
          <div className="flex justify-end gap-2">
            ...
            <DeleteButton
              ...
              message={list("deleteConfirm", { name: service.title })}
            />
          </div>
        )}
      />
