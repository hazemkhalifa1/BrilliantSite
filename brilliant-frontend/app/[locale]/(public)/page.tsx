...
import type { HeroSection, PagedResult, Project, Service } from "@/types";
...
async function getHomeData() {
  ...
  return { hero, services, projects };
}

export default async function HomePage() {
  const t = await getTranslations("home");
  const locale = await getLocale();
  const { hero, services, projects } = await getHomeData();
  ...
  return (
    <div>
      ...
      />
      {/* Hero */}
      <header className="relative h-[85vh] w-full overflow-hidden border-b-4 border-black">
        <div
        ...
        <div className="relative z-10 flex h-full flex-col justify-center container-brilliant">
          <Reveal className="max-w-3xl">
            <div className="mb-6 inline-block bg-tertiary px-3 py-1 font-headline text-sm font-bold uppercase tracking-tighter text-white">
              {t("eyebrow")}
            </div>
            ...
            <p className="mb-10 max-w-xl border-s-4 border-tertiary py-2 ps-6 text-xl text-white">
              {localized(locale, hero?.subText || t("defaultSubtext"), hero?.subTextAr)}
            </p>
            <div className="flex flex-wrap gap-6">
              <Link
                href={hero?.primaryBtnUrl || "/projects"}
                className="flex items-center gap-3 border-2 border-black bg-[#BA1A1A] px-10 py-5 font-headline text-lg font-bold uppercase tracking-tighter text-white shadow-[6px_6px_0px_black] transition-all hover:translate-x-[6px] hover:translate-y-[6px] hover:shadow-none"
              >
        ...
        <Reveal className="absolute bottom-12 end-4 z-10 hidden lg:block xl:end-12" delay={200}>
          <div className="border-4 border-black bg-white p-6 shadow-[8px_8px_0px_#ba1a1a]">
            <div className="monospaced space-y-2 text-xs font-bold text-black">
              <p className="text-tertiary">{t("metricsLabel")}</p>
              {metrics.map(([label, value]) => (
                <p key={label}>
                  {label}: <span dir="ltr">{value}</span>
                </p>
              ))}
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="brutalist-card h-full bg-white p-8">
                      <div
                        className={`mb-8 flex h-14 w-14 items-center justify-center border-2 border-black ${style.bg}`}
                      >
      ...
      <section className="overflow-hidden border-b-4 border-black bg-surface py-24">
        <div className="container-brilliant grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
          <Reveal className="relative order-2 lg:order-1">
            <div
              className="mx-auto aspect-square max-w-md border-4 border-black bg-cover bg-center shadow-[16px_16px_0px_#0059bb]"
              style={{ backgroundImage: `url('${ABOUT_IMAGE}')` }}
            />
            <div className="absolute -left-6 -top-6 -z-10 h-32 w-32 border-2 border-black bg-tertiary" />
          </Reveal>
          ...
          <div className="container-brilliant">
            <Reveal className="mb-16">
              ...
              <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
                ...
                <Link
                  href="/projects"
                  className="inline-flex items-center gap-3 border-b-4 border-black pb-1 font-headline text-sm font-bold uppercase tracking-tighter text-black transition-colors hover:border-tertiary hover:text-tertiary"
                >
                  {t("viewAllProjects")}
              ...
              {projects.map((project, index) => (
                <Reveal key={project.id} delay={index * 100}>
                  <Link
                    ...
                    className="group block border-2 border-black bg-white p-4 transition-colors hover:bg-surface-container"
                  >
